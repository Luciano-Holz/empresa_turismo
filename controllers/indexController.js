...
const Pessoa = require("../models/indexPessoa");
...
app.get("/edit-motorista/:id", (req, res) => {
  Motorista.findOne({ where: { id: req.params.id } })
    .then((motorista) => {
...
app.post("/motorista-edit", (req, res) => {
  Motorista.findOne({ where: { id: req.body.id } })
    .then((motorista) => {
...
app.get("/edit-carro/:id", (req, res) => {
  Carro.findOne({ where: { id: req.params.id } })
    .then((carro) => {
...
app.post("/carro-edit", (req, res) => {
  Carro.findOne({ where: { id: req.body.id } })
    .then((carro) => {
...
//rota para mostrar todas as viagens sem poder editar e deletar
app.get("/index", (req, res) => {
  Viagem.findAll().then((viagem) => {
    res.render("index", { viagem: viagem });
  });
});
...
app.get("/edit-viagem/:id", (req, res) => {
  Viagem.findOne({ where: { id: req.params.id } })
    .then((viagem) => {
...
app.post("/viagem-edit", (req, res) => {
  Viagem.findOne({ where: { id: req.body.id } })
    .then((viagem) => {
...
///////////////////////
//rota para mostrar todas as pessoas
app.get("/pessoa", (req, res) => {
  Pessoa.findAll().then((pessoas) => {
    res.render("pessoa", { pessoas: pessoas });
  });
});
//rota para mostrar a página pessoa
app.get("/pessoa", (req, res) => {
  res.render("pessoa");
});
...
app.post("/pessoa", (req, res) => {
  Pessoa.create({
    ...
    .catch((erro) => {
      console.log("Erro: Cadastro de pessoa nao realizado!" + erro);
      res.send("Erro: Cadastro de pessoa nao realizado!" + erro);
    });
});

//rota receber viagem pelo id
app.get("/edit-pessoa/:id", (req, res) => {
  Pessoa.findOne({ where: { id: req.params.id } })
    .then((pessoa) => {
      res.render("edit-pessoa", { pessoa: pessoa.toJSON() });
    })
    .catch((erro) => {
      res.redirect("/pessoa" + erro);
    });
});
//rota para atualizar viagem
app.post("/pessoa-edit", (req, res) => {
  Pessoa.findOne({ where: { id: req.body.id } })
    .then((pessoa) => {
...
app.get("/del-pessoa/:id", (req, res) => {
  Pessoa.destroy({
    ...
    .catch(() => {
      res.send("Pessoa não apagada!");
    });
});
///////////////////
//rota para mostrar a página login
app.get('/login', (req, res) => {
  res.render('login')
});
//rota para fazer login
app.post('/login', async (req, res) => {
  const login = await Pessoa.findOne({ where: { email: req.body.email, senha: req.body.senha } })
  if (!login) {
    res.status(400).send(`Dados não encontrados na base de dados.`)
    res.render('login')
  } else {
    res.redirect("/pessoa");
  }
